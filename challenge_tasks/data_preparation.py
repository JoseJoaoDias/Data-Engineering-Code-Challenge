...
def validation(df: DataFrame, df_name: str) -> DataFrame:
    ...
    if set(actual_schema.keys()) != set(expected_schema_dict.keys()):
        ...
        logging.error(
            f"Schemas do not match for {df_name} dataset. Missing columns: {missing_columns}, Extra columns: {extra_columns}"
        )
    else:
    ...
    for field in expected_schema:
        if actual_schema[field.name] != field.dataType:
            logging.warning(
                f"Casting column {field.name} from {actual_schema[field.name]} to {field.dataType}"
            )

            if isinstance(
                field.dataType, DateType
            ):  # Check if the field data type is DateType and process it to handle potential null values
                df = df.withColumn(
                    field.name,
                    coalesce(
                ...
                df = df.withColumn(
                    field.name,
                    date_format(col(field.name), "yyyy-MM-dd").cast(field.dataType),
                )
            else:
